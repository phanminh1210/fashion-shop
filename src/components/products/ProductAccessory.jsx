...
export default function ProductAccessory() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8111/api/v1/categories/products/by-category/3')
            .then(res => {
                setProducts(res.data.content); // lấy mảng sản phẩm từ response
            })
            .catch(err => {
                console.error("Lỗi khi lấy sản phẩm Giày:", err);
            });
    }, []);
