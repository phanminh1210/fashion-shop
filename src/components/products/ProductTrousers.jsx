...
export default function ProductTrousers() {
    const [products, setProducts] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        axios.get('http://localhost:8111/api/v1/categories/products/by-category/2')
            .then(res => {
                setProducts(res.data.content); // chỉ lấy mảng content từ response
            })
            .catch(err => {
